...
def nmpc(
    ...
    for i in range(0, num_horizon):
        ...
        # cost += 500 * (acc**2) + 500*((acc-acc_ego_old)**2)
        ...
        # cost = cost + 50 * (acc**2)# + (X[X_DIM_W * (i + 1) + X_ID_W["v"]] - spd_limit)**2
    # cost = cost - 10 * (X[X_DIM_W * (num_horizon) + X_ID_W["x"]] - X[X_DIM_W * (0) + X_ID_W["x"]]) ** 2

    # constraint on position w.r.t. the preceding vehicle
        
    # x(t) < x_max
    t0 = 1.5
    gamma_min = SX.sym("gamma_min", num_horizon)
    for i in range(0, num_horizon):
        constraint = vertcat(
            constraint,
            X[X_DIM_W * (i + 1) + X_ID_W["x"]]
            - (pos_pred_max_preced[i + 1] - t0 * X[X_DIM_W * (i + 1) + X_ID_W["v"]]) - gamma_min[i],
        )
        cost += 10000*(gamma_min[i]**2)
    # x(t) >= x_min
    if max_spacing_constraint:
        gamma_max = SX.sym("gamma_max", num_horizon)
        for i in range(0, num_horizon):
            constraint = vertcat(
                constraint,
                X[X_DIM_W * (i + 1) + X_ID_W["x"]] - (pos_pred_min_preced[i + 1] - t0*X[X_DIM_W * (i + 1) + X_ID_W["v"]]) + gamma_max[i],
            )
            cost += 10000*(gamma_max[i]**2)
    ...
    if not terminal_constraint:
        if not max_spacing_constraint:
            nlp = {
                "x": vertcat(X, U, gamma_min),
                "f": cost,
                "g": constraint,
            }
        else:
            nlp = {
                "x": vertcat(X, U, gamma_min, gamma_max),
                "f": cost,
                "g": constraint,
            }
    else:
        if not max_spacing_constraint:
            nlp = {
                "x": vertcat(X, U, gamma_min, gamma_x, gamma_v),
                "f": cost,
                "g": constraint,
            }
        else:
            nlp = {
                "x": vertcat(X, U, gamma_min, gamma_max, gamma_x, gamma_v),
                "f": cost,
                "g": constraint,
            }
    ...
    if not terminal_constraint:
        if not max_spacing_constraint:
            lbx = x + [-inf, 0] * (num_horizon) + [-20] * (num_horizon) + [0] * (num_horizon)
            ubx = (
                x
            ...
            lbx = x + [-inf, 0] * (num_horizon) + [-20] * (num_horizon)  + [0] * (num_horizon) + [0]* num_horizon
            ubx = (
                x
                + [inf, spd_limit] * (num_horizon)
                + [100] * (num_horizon)
        ...
        if not max_spacing_constraint:
            lbx = x + [-inf, 0] * (num_horizon) + [-20] * (num_horizon)  + [0] * (num_horizon) + [0] + [0]
            ubx = (
                x
                + [inf, spd_limit] * (num_horizon)
                + [100] * (num_horizon)
            ...
            lbx = x + [-inf, 0] * (num_horizon) + [-20] * (num_horizon) + [0] * (num_horizon) + [0] * (num_horizon) + [0] + [0]
            ubx = (
                x
                + [inf, spd_limit] * (num_horizon)
                + [100] * (num_horizon)
    ...
    if max_spacing_constraint:
        lbg_dynamics = (lbg_dynamics+ [0] * num_horizon)
        ubg_dynamics = (ubg_dynamics+ [inf] * num_horizon)
    ...
    if terminal_constraint:
        if not max_spacing_constraint:
            x_guess = np.zeros(((num_horizon + 1) * X_DIM_W + num_horizon * U_DIM_W + 2 + num_horizon))
        else:
            x_guess = np.zeros(((num_horizon + 1) * X_DIM_W + num_horizon * U_DIM_W + num_horizon + 2 + num_horizon))
    else:
        if not max_spacing_constraint:
            x_guess = np.zeros(((num_horizon + 1) * X_DIM_W + num_horizon * U_DIM_W + num_horizon))
        else:
            x_guess = np.zeros(((num_horizon + 1) * X_DIM_W + num_horizon * U_DIM_W + num_horizon + num_horizon))
    ...
    # print(x_sol[num_horizon, X_ID_W["x"]] - (pos_stop_bar))
    
    # print('tl constraint', tl_constraint)
    # print('terminal constraint', terminal_constraint)
    # print('max spacing contraint', max_spacing_constraint)
    # fig, axs = plt.subplots(3)
    
    # axs[0].plot(np.arange(0,15.2,0.2),x_sol[:, X_ID_W["x"]])
    ...
    # axs[0].plot(np.arange(0,15.2,0.2), rl_pos_bound, 'r')
    # # axs[0].plot(pos_pred_max_preced[i + 1] - t0 * X[X_DIM_W * (i + 1) + X_ID_W["v"]],'b')
    # # plt.show()
    # axs[0].set_ylim(pos_ego-100, pos_ego+1000)
    
    
    # axs[1].plot(np.arange(0,15.2,0.2),x_sol[:, X_ID_W["v"]])
    
    # axs[1].set_xlabel("Time (s)")
    # axs[1].set_ylabel("Speed (m/s)")
    # axs[1].set_ylim(0, spd_limit + 1)
    # # plt.show()
    
    
    # axs[2].plot(np.arange(0,15,0.2),u_sol[:, U_ID_W["sig"]])
    
    # axs[2].set_xlabel("Time (s)")
    # axs[2].set_ylabel("Warning")
    # axs[2].set_ylim(-20.1, 100.1)
    # plt.show()
